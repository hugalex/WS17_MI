...
void MouseFeatures::transform(ITransformer::info info, ssi_stream_t &stream_in, ssi_stream_t &stream_out, ssi_size_t xtra_stream_in_num, ssi_stream_t xtra_stream_in[])
{
	...
	if (_options.frame_min_x) {								//compute frame_min_x
	...
	if (_options.frame_max_x) {								//compute frame_max_x
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.frame_average_x) {							//compute frame_average_x
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.frame_min_y) {								//compute frame_min_y
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.frame_max_y) {								//compute frame_max_y
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.frame_average_y) {							//compute frame_average_y
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.global_min_x) {							//compute global_min_x
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.global_max_x) {							//compute global_max_x
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.global_average_x) {						//compute global_average_x
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.global_min_y) {							//compute global_min_y
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.global_max_y) {							//compute global_max_y
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.global_average_y) {						//compute global_average_y
		for (int j = 0; j < stream_in.num; j++) {
	...
	if (_options.distance) {								//compute distance
	...
	if (_options.velocity) {								//compute velocity
		float buf_value_x = in[0];
		float buf_value_y = in[1];
	...
	if (_options.variability) {								//compute variability
		...
		out[out_index++] = abs(variability_x);
		out[out_index++] = abs(variability_y);
	}

	if (_options.standard_deviation) {						//compute standard_deviation
		float variability_x = 0;
		float variability_y = 0;
		float average_x = 0;
		float average_y = 0;
		...
		standard_deviation_x = sqrt(abs(variability_x));
		standard_deviation_y = sqrt(abs(variability_y));
